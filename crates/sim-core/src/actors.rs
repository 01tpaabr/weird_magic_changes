...
mod tests {
    ...
    fn validate_catches_every_broken_invariant() {
        ...
        assert!(validate(o, &bad, m, 1).unwrap_err().contains("dead"));
        let mut bad = p.clone();
        bad[1].cell = 3;
        assert!(validate(o, &bad, m, 1).unwrap_err().contains("expects"));
        let mut bad = p.clone();
        bad[1].cell = u16::MAX;
        assert!(validate(o, &bad, m, 1).unwrap_err().contains("outside"));
        let mut bad_o = *o;
        bad_o[7] = ActorId::pack(SEED, 0);
        assert!(
            validate(&bad_o, p, m, 1)
                .unwrap_err()
